    ...
    <div class="card">
        ...
        <Toolbar class="mb-4">
                <template v-slot:start>
                    <Button label="New Account" icon="pi pi-plus" class="p-button-success mr-2" @click="openNewHeader" />
                </template>
        ...
        <DataTable :value="products" :rowClass="rowClass" :rowStyle="rowStyle" tableStyle="min-width: 50rem" class="p-datatable-sm">
        ...
        <Dialog v-model:visible="accountDialog" :style="{width: '450px'}" header="New Account" :modal="true" class="p-fluid">
            ...
            <div class="field">
                <label for="accountCost">Account Cost {{ account.account_cost }}</label>
                <SelectButton v-model="account.account_cost" :options="trueFalse" optionLabel="name" optionValue="id" aria-labelledby="basic" />
            </div>
            <div class="field">
                <label for="costCenter">Cost Center {{ account.cost_center }}</label>
