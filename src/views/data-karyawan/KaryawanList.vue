...
const breadcrumbItems = ref([{ label: 'Data Karyawan' }, { label: 'List' }]);
...
const userDialogEndDate = ref(false);
const submittedEndDate = ref(false);
const editedEndDate = ref(false);
...
const hideDialog = () => {
    ...
    userDialogEndDate.value = false;
    submittedEndDate.value = false;
};
...
const saveUserEndDate = () => {
    submittedEndDate.value = true;

    if (user.value.name.trim()) {
        // Pastikan end_date diubah menjadi format "YYYY-MM-DD"
        if (user.value.end_date instanceof Date) {
            const localDate = new Date(user.value.end_date.getTime() - user.value.end_date.getTimezoneOffset() * 60000);
            user.value.end_date = localDate.toISOString().split("T")[0];
        }

        // Periksa apakah tanggal sekarang sudah melewati end_date
        const today = new Date();
        const endDate = new Date(user.value.end_date);
        if (today > endDate) {
            user.value.status = "0"; // Atur status ke 0 jika sudah lewat end_date
        } else {
            user.value.status = selectedStatus.value.value
                ? selectedStatus.value.value
                : user.value.status;
        }

        // Kirim data untuk diperbarui
        updateListData(user.value, user.value.id);

        // Reset nilai dan tutup dialog
        editedEndDate.value = false;
        user.value = {};
    }
};
...
// Computed untuk menentukan apakah dropdown harus disabled
const isStatusDisabled = computed(() => {
    if (!user.value.end_date) return false; // Jika tidak ada tanggal, dropdown tidak di-disable
    const today = new Date();
    const endDate = new Date(user.value.end_date);
    return today > endDate; // Disable jika tanggal saat ini melewati end_date
});

// Periksa dan ubah status otomatis jika tanggal sudah lewat
const checkAndUpdateStatus = () => {
    ...
    if (today > endDate) {
        user.value.status = "0"; // Atur status ke 0 jika sudah lewat end_date
        selectedStatus.value = { label: "No Active", value: "0" }; // Perbarui dropdown
    }
};

// Jalankan pemeriksaan saat komponen dimuat atau data diubah
onMounted(() => {
    if (user.value.end_date) {
        checkAndUpdateStatus();
    }
});
watch(() => user.value.end_date, () => {
    checkAndUpdateStatus();
});
</script>
...
<template>
    <div class="grid">
        <div class="col-12">
            <div class="card card-w-title">
                ...
                <Toolbar class="mb-4">
                    <template v-slot:start>
                        <div class="my-2">
                            <Button label="Tambah Data" icon="pi pi-plus" class="p-button-success mr-2"
                                @click="openNew" />
                        </div>
                    </template>
                ...
                <DataTable ref="dt" :value="Users" responsiveLayout="scroll">
                    ...
                    <Column field="aksi" header="Aksi" :exportable="false" style="min-width:10rem">
                        <template #body="slotProps">
                            <Button icon="pi pi-pencil" outlined rounded class="mr-2"
                                @click="editUser(slotProps.data)" />
                            <Button icon="pi pi-sign-out" outlined rounded severity="danger" class="mr-2"
                                @click="editUserEndDate(slotProps.data)" />
            ...
            <Dialog v-model:visible="userDialog" :style="{ width: '450px' }" header="Data Karyawan" :modal="true"
                class="p-fluid">

                <div class="field">
                    <label for="number_officer">NIP</label>
                ...
                <div class="field">
                    ...
                    <InputText id="id_card" v-model.trim="user.id_card" required="true" autofocus
                        :class="{ 'p-invalid': submitted && !user.id_card }" />
                    <small class="p-error" v-if="submitted && !user.id_card">NIK KTP is required.</small>
                </div>

                <!-- <div class="field">
                    <label for="number_officer">NIP</label>
                    <InputText id="number_officer" v-model.trim="user.number_officer" required="true" autofocus
                        :class="{ 'p-invalid': submitted && !user.number_officer }" />
                    <small class="p-error" v-if="submitted && !user.number_officer">NIP is required.</small>
                </div> -->
                <div class="field">
                ...
                <div class="field">
                    ...
                    <Dropdown inputId="gender" v-model="user.gender" :options="StatusGenderList" optionLabel="label"
                        optionValue="id" placeholder="Select a Jenis Kelamin" required autofocus
                ...
                <div class="field">
                    ...
                    <Dropdown inputId="class" v-model="selectedPosition" :options="positionList" optionLabel="alias"
                        optionValue="id" placeholder="Select a Position" required="true" autofocus
                        :class="{ 'p-invalid': submitted && !selectedPosition }" />
                    <small class="p-error" v-if="submitted && !selectedPosition">Position is required.</small>
                </div>
                <div class="field">
                ...
                <div class="field" v-if="!edited">
                    ...
                    <FileUpload mode="basic" accept="image/*" v-model.trim="user.photo" :maxFileSize="1000000"  required="true" toggleMask autofocus
                        @select="convertToBase64" :class="{ 'p-invalid': submitted && !user.photo }" />
                    <small class="p-error" v-if="submitted && !user.photo">Foto Pass is required.</small>
                </div>
                <div class="field" v-if="edited">   
                    <label for="photo">Foto Pass</label>
                    <FileUpload mode="basic" accept="image/*" v-model.trim="user.photo" :maxFileSize="1000000"  required="true" toggleMask autofocus
            ...
            <Dialog v-model:visible="editedEndDate" :style="{ width: '450px' }" header="Data Karyawan" :modal="true"
                class="p-fluid">

                <div class="field">
                    <label for="end_date">End Date</label>
                    <Calendar id="end_date" v-model.trim="user.end_date" required dateFormat="yy-mm-dd" maxlength="10"
                        :class="{ 'p-invalid': submittedEndDate && !user.end_date }" />
                </div>

                <div class="field">
                    <label for="status" class="mb-3">Status</label>
                    <Dropdown id="status" v-model="selectedStatus" :options="statuses" optionLabel="label"
                        placeholder="Select a Status" :disabled="isStatusDisabled">
                        <template #value="slotProps">
                            <div v-if="slotProps.value && slotProps.value.value">
                                <Tag :value="slotProps.value.label" :severity="getStatusLabel(slotProps.value.value)" />
                            </div>
                ...
                <template #footer>
                    <Button label="Cancel" icon="pi pi-times" text @click="hideDialog" />
                    <Button label="Save" icon="pi pi-check" text @click="saveUserEndDate" />
                </template>
            </Dialog>
        </div>
    </div>
