...
const setData = () => {
    ...
    // inputForm.document_no = `{no}/EXT.${props.formData.category.alias}/${dataLocalStorage.divisi_alias}/${romanize(currentTime.getMonth() + 1)}/${currentTime.getFullYear()}`
...
const setTextSigner = (data) => {
    ...
    for (let i = 0; i < data.length; i++) {
        ...
        html += `<td><p class="mb-0" style="${element.position !== 'External' ? 'text-decoration: underline;' : ''} font-weight: bold;">${element.user_name}</p>
            ${element.position !== 'External' ? element.position : ''}</td>`;
    }
    html += `</tr></table>`;
    return html;
}
            ...
            <template v-slot:content>
                ...
                <div class="p-fluid formgrid grid mt-3">
                        ...
                        <div class="p-inputgroup flex-1">
                            ...
                            >
                                <template #value="slotProps">
                                    <div v-tooltip.bottom="slotProps.value.address" v-if="slotProps.value" class="flex align-items-center">
                                ...
                                <template #option="slotProps">
                                    <div v-tooltip.bottom="slotProps.option.address" class="flex align-items-center py-2 px-4">
                            ...
                            <!-- <template> -->
                                <div class="flex justify-content-center p-fluid">
                                    <div v-focustrap class="cards">
                                        ...
                                        <div class="field">
                                            <InputText id="input" v-model="inputFormTujuan.name" type="text" placeholder="Name" autofocus />
                                        </div>
                                        <div class="field">
                                            <Textarea v-model="inputFormTujuan.alamat" rows="5" cols="30" placeholder="Alamat" />
                                        </div>
                    ...
                    <div class="field col-12 md:col-6">
                        ...
                        <InputText for="up" type="text" v-model="inputForm.up" required="true" autofocus :class="{'p-invalid': submitted && !inputForm.up}" />
                        ...
                        <div class="p-fluid formgrid grid">
                            <div class="field col-12 md:col-1 mb-0 align-self-end">
                                ...
                                <div class="switch-Tandatangan">
                                    <InputSwitch @change="changePenandatanganExternal()" v-model="penandatanganExternalSwitch"/>
                                </div>
                            </div>
                            <div class="field col-12 md:col-11 mb-0" v-if="penandatanganExternalSwitch">
                                <MultiSelect
                                    inputId="signer"
                                    v-model="selectSignerExternal"
                                    display="chip"
                                    :options="optionSignerExternal"
                                    @change="setSignerExternal($event)"
                                    optionLabel="user_name"
                                    placeholder="Pilih Penandatangan"
                                    required="true" autofocus :class="{'p-invalid': submitted && !selectSignerExternal}"
                                    />
                                <small class="p-error" v-if="submitted && !selectSignerExternal">Penandatangan is required.</small>
                            </div>
                        </div>
                    </div>
                    ...
                    <template v-for="(item, idx) in dataForm" :key="idx" >
                        <div class="field col-12 md:col-6 mb-0">
                            ...
                            <Calendar v-if="item.type==='2' || item.type==='5'" :id="item.parameter" v-model="item.value" dateFormat="dd/mm/yy" />
                            <MultiSelect
                                v-if="item.type==='3'"
                        ...
                        <div v-if="item.type==='6'" class="field col-12 md:col-12">
                            <Message v-if="formData.template.parameter[0].value !== ''" class="w-2 mt-0" severity="warn" icon="pi pi-file" :closable="false">File sudah ada</Message>
                            <FileUpload mode="basic" customUpload @select="onSelectFileTandatanganBasah" accept="application/pdf" :maxFileSize="1000000" />
                        </div>
