        ...
        <DataTable ref="dt" :value="SuratData" responsiveLayout="scroll">
            ...
            <Column header="Aksi" :exportable="false" style="width: 100%; min-width:8rem">
                <template #body="slotProps">
                    <Button
                        v-if="slotProps.data.sender.user_id === dataLocalStorage.target && slotProps.data.status === '1'"
                        icon="pi pi-pencil" outlined rounded class="mr-2" @click="viewSuratRevisi(slotProps.data)" />
                    <Button icon="pi pi-trash" outlined rounded severity="danger"
                        @click="cancelSuratOpen(slotProps.data)" />
                </template>
            </Column>
