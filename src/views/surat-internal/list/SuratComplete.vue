...
const cancelSuratComplete = (data) => {
    ...
    <div>
        ...
        ></Column> -->
            <Column header="Aksi" :exportable="false" expander headerStyle="min-width:14rem">
                <template #body="slotProps">
                    <Button icon="pi pi-eye" outlined rounded class="mr-2 "
                        @click="viewSuratCompleteInternal(slotProps.node)" />
                    <Button v-if="slotProps.node.parent === 0" icon="pi pi-reply" outlined rounded class="mr-2 "
                        severity="secondary" @click="replySuraInternal(slotProps.node)" />
                    <Button v-if="divisiRole.divisi === 1" icon="pi pi-trash" outlined rounded severity="danger"
                        class="" @click="cancelSuratComplete(slotProps.node)" />
                </template>
            </Column>
            ...
            <Column field="receiver" header="Tujuan">
                <template #body="slotProps">
                    <template v-if="slotProps.node.receiver.length > 3">
                        <template v-if="showAll(slotProps.node.id)">
                            ...
                            <a @click="toggleShowAll(slotProps.node.id)">...</a>
                        </template>
                        <template v-else>
                            <template v-for="(item, idxTujuan) in slotProps.node.receiver.slice(0, 3)" :key="idxTujuan">
                                <Chip :label="item.user_name" class="mb-2 mr-2" />
                            </template>
                            <a @click="toggleShowAll(slotProps.node.id)">...</a>
                        </template>
            ...
            <Column field="Verifikator" header="Verifikator">
                <template #body="slotProps">
