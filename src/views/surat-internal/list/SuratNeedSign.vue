...
const cancelSuratSigner = (data) => {
    ...
    swalWithBootstrapButtons.fire({
        ...
        if (result.isConfirmed) {
            ...
            data.status = "7"
            suratService.putSurat(data).then((data) => {
                let page = (first.value/10)+1;
                let date = new Date();
                let currentDate = dateMonthYear(date);
                // filterMonth.value = date;
    ...
    <DataTable ref="dt" :value="SuratData" responsiveLayout="scroll">
        <Column header="Aksi" :exportable="false" style="min-width:8rem">
            <template #body="slotProps">
                ...
                <Button v-if="divisiRole.divisi === 1" icon="pi pi-times" outlined rounded severity="danger" @click="cancelSuratSigner(slotProps.data)" />
            </template>
        </Column>
        ...
        <Column field="status" header="Status" :sortable="false" headerStyle="width:14%; min-width:10rem;">
            <template #body="slotProps">
                <Badge :value="`Need Sign`" severity="warning" class="color-white"></Badge>
            </template>
        </Column>
        <template #footer>
